...
import { useFavouriteMutation } from "../../UserSlices/usersApiSlice.js";
import {toast} from 'react-toastify';
const FavouriteDoctorCard = ({doctor, setFavourites})=>{
  const [favourite, { isLoading }] = useFavouriteMutation();

  const handleFavourite = async () => {
    try {
      const res = await favourite(doctor?.doctorId?._id).unwrap();
  ...
  return (
    <div className="relative flex w-full max-w-[17rem] flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-lg h-auto mx-auto">
    <div className="relative mx-4 mt-4 overflow-hidden text-white shadow-lg rounded-xl bg-blue-gray-500 bg-clip-border shadow-blue-gray-500/40">
      <img src={doctor?.doctorId?.images} alt="ui/ux review check" className="h-32 w-full" />
    ...
    <div className="p-6 pt-3">
      <Link to={"/doctor-details/"+doctor.doctorId._id}>
      <button
...
export default FavouriteDoctorCard